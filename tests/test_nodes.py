...
class TestGenerateFeedback:
    def test_returns_feedback(self):
        ...
        with patch(
            "src.german_tutor.tutor.generate_response", return_value="Gut gemacht!"
        ):
...
class TestGenerateQuestion:
    def test_returns_question(self):
        ...
        with patch(
            ...
            return_value="Wie ist das Wetter?",
        ):
    ...
    def test_includes_avoid_in_prompt_when_previous_questions(self):
        state = make_state(
            user_topic="weather", asked_questions=["Wie ist das Wetter?"]
        )
        with patch(
            "src.german_tutor.tutor.generate_response", return_value="Ist es kalt?"
        ) as mock:
